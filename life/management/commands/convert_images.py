...
class Command(BaseCommand):
	def convert_images(self):
		basedir = settings.ProjectDir + '/media/images/properties/'
		...
		for image in no_thumbs:
			call('convert {0} -background white -gravity center -resize {1} -extent {1} {2}'.format(basedir + image, 'x85', basedir + 'detail-small/' + image).split(' '))
			print 'Saved image', image, 'for detail page(small)'
			
		# Scaling for the large images on the detail page
		for image in no_thumbs:
			call('convert {0} -background white -gravity center -resize {1} -extent {1} {2}'.format(basedir + image, '698x382', basedir + 'detail-big/' + image).split(' '))
